...
export default function VerticalRaceGame({ socket: externalSocket, wallet, onClose, partyId }) {
  ...
  // For party game, we don't need to hide footer (it's already in a container)
  useEffect(() => {
    // Only hide footer if not in party context (no onClose prop means standalone page)
    if (!onClose) {
      setFooterHeight(0);
    }
  }, [onClose]);
  ...
  // Hide bottom nav when game component is active (only for standalone page, not party)
  useEffect(() => {
